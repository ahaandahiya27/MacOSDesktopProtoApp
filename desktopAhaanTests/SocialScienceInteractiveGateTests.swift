...
final class SocialScienceInteractiveGateTests: XCTestCase {
    ...
    /// (India's-neighbours explorer), ssch17 (home-to-many explorer), ssch08
    /// (sacred-geography explorer), a chronology chapter with a usable timeline,
    /// OR a chapter with a usable glossary (the default match challenge). No
    /// chapter may fall through to nothing. (As of cycle 72 every non-history
    /// chapter has a bespoke widget; the glossary fallback remains as a safety
    /// net pinned by this test.)
