...
- **40 lints clean** on every push — see `scripts/check_*.py`. Wired through `scripts/ci-build-test.sh` + the pre-commit hook. (Plus four commit-time ratchets: `check_critical_uitest_presence.py` + `check_uitest_label_coverage.py` (T2), and `check_designtokens_spacing.py` + `check_designtokens_radius.py` (J8), in the pre-commit chain.) Newest two (both 2026-06-13): `check_color_rgb_centralized.py` — freezes the `Color(red:green:blue:)` outside-ChapterTheme site count at 85 (audit baseline) so new RGB drift fails commit; can be ratcheted DOWN as a future sweep consolidates sites into `DesignTokens.BrandColor.*`. `check_a11y_identifier_uniqueness.py` — flags same-file `.accessibilityIdentifier(literal)` collisions so the 270 stable XCUITest selectors added in the 2026-06-12 sweep stay non-ambiguous; 206 literal-id sites scanned across 73 files. Earlier ratchets: `check_designtokens_spacing.py` + `check_designtokens_radius.py` (2026-06-12) — J8 regression ratchets flagging any raw integer literal in the spacing canon {2,4,8,12,16,24,32,48} or radius canon {8,10,14,16,22,999} outside `DesignTokens.{Spacing,Radius}.*`; scans 2,442 spacing + 389 radius sites on every commit + push. Earlier sweep: `check_inline_modifier_math.py` (2026-06-04) — finer-grained sister of `check_viewbuilder_depth.py`. Where the depth lint flags DENSE GeometryReader closures, this one flags any individual `.frame(...)` / `.position(...)` / `.offset(...)` / `.padding(...)` whose arg list contains inline arithmetic (`w * 0.3`, `cx - h * 0.16`) — the per-call form of the same Swift-5.5 `Segmentation fault: 11` class. Fix: hoist the expression to a typed `let _: CGFloat = ...` local above the surrounding result-builder closure. After the 2026-06-04 sweep, ~244 sites across 80 files were de-arithmeticized (16 ShapeDiagrams + Discover Scenes + Components + Tour/Sandbox surfaces + Progress views). See `ccd011a`, `162a71b`, plus the follow-up sweep commit.
...
- **v8.1 J8 + H2 + T3 close-out (2026-06-12):** ~3,150 padding/spacing + ~340 corner-radius literals routed through `DesignTokens.{Spacing,Radius}` (Waves 1–6); ~169 `.accessibilityHint(...)` added across ~89 files (H2 → ✅); 28 `.help(...)` tooltips on every menu command in `desktopAhaanApp.swift`; 2 new ratchets (`check_designtokens_spacing` + `check_designtokens_radius`) prevent regression. `a11y_labels` at **exact 100%** (706/706 after `afa10ee` hoisted a long-action Button to a helper so the chained label fell within the lint window). T3 source written at `desktopAhaanUITests/NavigationSmokeUITests.swift` but **still 🟡 — needs an Xcode "Add Files…" step** to wire it into the UITests target's pbxproj (the file is on disk but not yet in the build). See `J8_DESIGN_TOKENS_LEDGER.md`. iMac visual-verify still required for final close-out.
- **Test surface:** 835 XCTest methods + 66 swift-testing + 42 XCUITest in the target (NavigationSmokeUITests source landed on disk 2026-06-11; pending pbxproj add). Existing UITest count: 14 propagated-interactive walks [4 tours + 9 Build-a-* sandboxes + HomeExperiments, 2026-06-02] + 15 maths-Discover walks + 13 prior. Default CI runs unit tests only; UI tests are `--ui` opt-in (AX grant required on runner). T2 surfaces are guarded AX-free by the four commit-time ratchets above.
