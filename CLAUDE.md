...
- **v8.1 J8 + H2 + T3 close-out (2026-06-12):** ~3,150 padding/spacing + ~340 corner-radius literals routed through `DesignTokens.{Spacing,Radius}` (Waves 1–6); ~169 `.accessibilityHint(...)` added across ~89 files (H2 → ✅); `NavigationSmokeUITests` walks home → chapter → topic → concept → question end-to-end (T3 → ✅); 28 `.help(...)` tooltips on every menu command in `desktopAhaanApp.swift`; 2 new ratchets (`check_designtokens_spacing` + `check_designtokens_radius`) prevent regression. `a11y_labels` at **100%** (705/706). See `J8_DESIGN_TOKENS_LEDGER.md`. iMac visual-verify still required for final close-out.
- **Test surface:** 835 XCTest methods + 66 swift-testing + 43 XCUITest (incl. `NavigationSmokeUITests`, 2026-06-11; 14 propagated-interactive walks [4 tours + 9 Build-a-* sandboxes + HomeExperiments, 2026-06-02] + 15 maths-Discover walks + 13 prior). Default CI runs unit tests only; UI tests are `--ui` opt-in (AX grant required on runner). T2 surfaces are guarded AX-free by the four commit-time ratchets above.
