...
struct ExtraReadingRow: View {
    ...
    private var rows: [(label: String, suffix: String, systemImage: String, accentColor: Color, hint: String)] {
        if pack.id == "socialscience_class7" {
            return socialScienceRows
        }
        return [
    ...
    /// Social Science ships a tailored chapter-level article set keyed off
    /// the pack's enrichment arrays (generated by
    /// scripts/generate_socialscience_articles.py). The "Scientist
    /// Spotlight" / "Story Mode" surfaces don't fit History/Civics/Geo/Eco,
    /// so this list swaps them for an Overview, Beyond the Book, Common
    /// Mistakes and Timeline. Each chip still auto-hides when its article
    /// isn't bundled, so the row degrades gracefully.
    private var socialScienceRows: [(label: String, suffix: String, systemImage: String, accentColor: Color, hint: String)] {
        return [
            (label: "Chapter Overview",
             suffix: "_overview",
             systemImage: SFSymbolCompat.name("map.fill"),
             accentColor: Color.compatIndigo,
             hint: "A map of the whole chapter — topics, key ideas, and where to go next."),
            (label: "Vocabulary Deck",
             suffix: "_glossary",
             systemImage: SFSymbolCompat.name("character.book.closed"),
             accentColor: Color.compatIndigo,
             hint: "Read-mode dictionary of the chapter's key terms, with Hindi names."),
            (label: "NCERT Q&A",
             suffix: "_ncert_qa",
             systemImage: SFSymbolCompat.name("doc.text.fill"),
             accentColor: Color.compatTeal,
             hint: "Worked model answers to the textbook questions for revision."),
            (label: "Beyond the Book",
             suffix: "_beyond",
             systemImage: SFSymbolCompat.name("graduationcap.fill"),
             accentColor: Color.compatPurple,
             hint: "Olympiad-tier extensions that take the chapter a few classes further."),
            (label: "Challenge Problems",
             suffix: "_olympiad",
             systemImage: SFSymbolCompat.name("flag.fill"),
             accentColor: Color.compatPurple,
             hint: "The chapter's hardest questions with full step-by-step worked solutions and the traps to avoid."),
            (label: "What If?",
             suffix: "_whatif",
             systemImage: SFSymbolCompat.name("lightbulb.fill"),
             accentColor: Color.compatPurple,
             hint: "Turning-point thought experiments — think first, then compare."),
            (label: "Common Mistakes",
             suffix: "_mistakes",
             systemImage: SFSymbolCompat.name("exclamationmark.triangle.fill"),
             accentColor: .orange,
             hint: "The traps that catch careful students, paired with what's actually true."),
            (label: "Mini Project",
             suffix: "_miniproject",
             systemImage: SFSymbolCompat.name("wrench.and.screwdriver.fill"),
             accentColor: Color.compatTeal,
             hint: "A hands-on activity that turns the chapter's idea into something you can do."),
            (label: "Timeline",
             suffix: "_timeline",
             systemImage: SFSymbolCompat.name("clock.fill"),
             accentColor: .orange,
             hint: "The chapter's big changes laid out as a sequence, start to finish."),
        ]
    }
    ...
    private func resolvedEntry(forKey key: String) -> ArticleEntry? {
        ...
        // sch01_…; Social Science uses ssch01_… (chapter.id already namespace-prefixed).
        // The Sanskrit legacy `ch01` vocab deck is gated off here so it can't accidentally
        // surface Science's ch01_* articles.
        let lookup: String
        switch pack.id {
