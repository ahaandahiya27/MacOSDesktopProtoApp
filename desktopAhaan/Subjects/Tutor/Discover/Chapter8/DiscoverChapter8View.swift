...
struct DiscoverChapter8View: View {
    ...
    private var sceneBuilders: [() -> AnyView] {
        [
            ...
            { AnyView(CycloneSurvivalQuizScene(onComplete: { score in self.markComplete(15, score: score, max: 4) })) },
...
private struct CycloneSurvivalQuizScene: View {
    let onComplete: (Int) -> Void
    private struct Q: Identifiable {
        let id: String; let prompt: String; let opts: [String]; let correct: Int
    }
    private let qs: [Q] = [
        Q(id: "q1", prompt: "Heard a cyclone warning. First action?",
          opts: ["Take photos of clouds", "Move to designated shelter", "Wait and watch"], correct: 1),
        Q(id: "q2", prompt: "Storing emergency food + water for how many days?",
          opts: ["12 hours", "1 day", "3+ days"], correct: 2),
        Q(id: "q3", prompt: "During the cyclone, where is safest indoors?",
          opts: ["Near windows", "Innermost room, away from glass", "On the roof"], correct: 1),
        Q(id: "q4", prompt: "After the eye passes, what happens?",
          opts: ["It's over", "Winds return from opposite direction — still danger", "Sunshine"], correct: 1)
    ]
    @State private var picks: [String: Int] = [:]
    private var score: Int { qs.reduce(0) { $0 + ((picks[$1.id] == $1.correct) ? 1 : 0) } }
    var body: some View {
        ScrollView { LazyVStack(spacing: 14) {
            Text("Cyclone Survival Quiz").font(.largeTitle.bold())
                .foregroundColor(DesignTokens.BrandColor.canvasText).padding(.top, 18)
