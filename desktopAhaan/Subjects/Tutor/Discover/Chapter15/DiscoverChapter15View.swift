...
private struct LightQuizScene: View {
    let onComplete: (Int) -> Void
    private struct Q: Identifiable {
        let id: String; let prompt: String; let opts: [String]; let correct: Int
    }
    private let qs: [Q] = [
        Q(id: "q1", prompt: "Image in a plane mirror is:",
          opts: ["Upside down", "Left-right swapped", "Same as original"], correct: 1),
        Q(id: "q2", prompt: "Sunlight passing through a prism makes:",
          opts: ["White stays white", "Rainbow (7 colours)", "Only red"], correct: 1),
        Q(id: "q3", prompt: "Concave lens (caves IN):",
          opts: ["Magnifies", "Reduces — used in -ve glasses", "Burns paper"], correct: 1),
        Q(id: "q4", prompt: "Sky looks blue because:",
          opts: ["Air is blue", "Blue light scatters most", "Earth reflects sea"], correct: 1)
    ]
    @State private var picks: [String: Int] = [:]
    private var score: Int { qs.reduce(0) { $0 + ((picks[$1.id] == $1.correct) ? 1 : 0) } }
    var body: some View {
        ScrollView { LazyVStack(spacing: 14) {
            Text("Light Quiz").font(.largeTitle.bold())
                .foregroundColor(DesignTokens.BrandColor.canvasText).padding(.top, 18)
