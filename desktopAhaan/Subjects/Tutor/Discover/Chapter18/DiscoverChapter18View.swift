...
struct DiscoverChapter18View: View {
    ...
    private var sceneBuilders: [() -> AnyView] {
        [
            ...
            { AnyView(WastewaterQuizScene(onComplete: { score in self.markComplete(18, score: score, max: 4) })) },
            { AnyView(Scene9_BossQuiz_Ch18(pack: self.pack, chapter: self.chapter, onComplete: { score in self.markComplete(19, score: score, max: 5) })) }
        ]
    }
...
private struct WastewaterQuizScene: View {
    let onComplete: (Int) -> Void
    private struct Q: Identifiable {
        let id: String; let prompt: String; let opts: [String]; let correct: Int
    }
    private let qs: [Q] = [
        Q(id: "q1", prompt: "What's in wastewater?",
          opts: ["Just water", "Water + dissolved + suspended waste", "Only soap"], correct: 1),
        Q(id: "q2", prompt: "What does anaerobic digestion of sludge produce?",
          opts: ["Pure water", "Biogas (methane)", "Salt"], correct: 1),
        Q(id: "q3", prompt: "Greywater is from:",
          opts: ["Toilets only", "Sinks + showers + washing", "Mixed with rain"], correct: 1),
        Q(id: "q4", prompt: "Indus Valley civilisation was famous for:",
          opts: ["No sanitation", "Brick-lined sewers", "Cars"], correct: 1)
    ]
    @State private var picks: [String: Int] = [:]
    private var score: Int { qs.reduce(0) { $0 + ((picks[$1.id] == $1.correct) ? 1 : 0) } }
    var body: some View {
        ScrollView { LazyVStack(spacing: 14) {
            Text("Wastewater Quiz").font(.largeTitle.bold())
                .foregroundColor(DesignTokens.BrandColor.canvasText).padding(.top, 18)
