...
struct DiscoverChapter19View: View {
    ...
    private var sceneBuilders: [() -> AnyView] {
        [
            ...
            { AnyView(AstronomyQuizScene(onComplete: { score in self.markComplete(18, score: score, max: 4) })) },
            { AnyView(Scene9_BossQuiz_Ch19(pack: self.pack, chapter: self.chapter, onComplete: { score in self.markComplete(19, score: score, max: 5) })) }
        ]
    }
...
private struct AstronomyQuizScene: View {
    let onComplete: (Int) -> Void
    private struct Q: Identifiable {
        let id: String; let prompt: String; let opts: [String]; let correct: Int
    }
    private let qs: [Q] = [
        Q(id: "q1", prompt: "How many planets in the Solar System?",
          opts: ["7", "8", "9"], correct: 1),
        Q(id: "q2", prompt: "Why do we have seasons?",
          opts: ["Distance from Sun", "Earth's axis is tilted 23.5°", "Moon's phases"], correct: 1),
        Q(id: "q3", prompt: "What's at the centre of the Solar System?",
          opts: ["Earth", "Sun", "Moon"], correct: 1),
        Q(id: "q4", prompt: "India's first Mars mission?",
          opts: ["Chandrayaan", "Mangalyaan", "Aditya"], correct: 1)
    ]
    @State private var picks: [String: Int] = [:]
    private var score: Int { qs.reduce(0) { $0 + ((picks[$1.id] == $1.correct) ? 1 : 0) } }
    var body: some View {
        ScrollView { LazyVStack(spacing: 14) {
            Text("Astronomy Quiz").font(.largeTitle.bold())
                .foregroundColor(DesignTokens.BrandColor.canvasText).padding(.top, 18)
