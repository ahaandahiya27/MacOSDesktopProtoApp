...
struct DiscoverChapter12View: View {
    ...
    private var sceneBuilders: [() -> AnyView] {
        [
            ...
            { AnyView(Scene9_BossQuiz_Ch12(pack: self.pack, chapter: self.chapter, onComplete: { score in self.markComplete(19, score: score, max: 5) })) }
        ]
    }
    ...
    var body: some View {
        ScrollView { LazyVStack(spacing: 14) {
            ...
            Text("🥥").font(.system(size: 100))
                ...
                .animation(.linear(duration: 4).repeatForever(autoreverses: true), value: floating)
