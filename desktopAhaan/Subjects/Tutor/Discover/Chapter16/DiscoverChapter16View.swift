...
private struct WaterQuizScene: View {
    let onComplete: (Int) -> Void
    private struct Q: Identifiable {
        let id: String; let prompt: String; let opts: [String]; let correct: Int
    }
    private let qs: [Q] = [
        Q(id: "q1", prompt: "What % of Earth's water is FRESH water humans can use?",
          opts: ["50%", "<1%", "97%"], correct: 1),
        Q(id: "q2", prompt: "Most water-saving irrigation method?",
          opts: ["Flood", "Drip", "Sprinkler"], correct: 1),
        Q(id: "q3", prompt: "Rainwater harvesting means…",
          opts: ["Buying water", "Collecting + storing rainwater", "Stealing rivers"], correct: 1),
        Q(id: "q4", prompt: "What kills germs in dirty water?",
          opts: ["Adding sugar", "Boiling for 5+ min", "Storing in plastic"], correct: 1)
    ]
    @State private var picks: [String: Int] = [:]
    private var score: Int { qs.reduce(0) { $0 + ((picks[$1.id] == $1.correct) ? 1 : 0) } }
    var body: some View {
        ScrollView { LazyVStack(spacing: 14) {
            Text("Water Quiz").font(.largeTitle.bold())
                .foregroundColor(DesignTokens.BrandColor.canvasText).padding(.top, 18)
