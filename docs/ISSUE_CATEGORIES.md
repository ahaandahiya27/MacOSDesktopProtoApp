...
| ID | Category | Status |
|----|----------|--------|
| A1 | Swift 5.5 ViewBuilder 10-child limit per closure | ✅ static audit clean, Group{} wraps where needed. 2026-05-31: `check_viewbuilder_limit.py` extended to also scan `CommandGroup`/`CommandMenu`/`Menu`/`ToolbarItemGroup` and `@ViewBuilder`/`@CommandsBuilder` funcs/vars (closed the gap that let the 20-child Help menu break the iMac on 2026-05-30); now a hard pre-commit gate with `--selftest` |
...
| A3 | No SF Symbols 3+/4+ names | ✅ ~38 symbols routed through SFSymbolCompat. 2026-05-17 follow-up: iMac runtime surfaced 3 unrouted symbols (`fork.knife`, `figure.2.and.child.holdinghands`, `pencil.and.ruler.fill`); defensive sweep added 2 more (`figure.run`, `gearshape.2`); fixed 7 direct-bypass call-sites; caught a cascading bug where `frying.pan.fill` fell back to also-modern `fork.knife`. 2026-05-19 follow-up: visual screenshot audit of Ch.7 surfaced blank instrument icons → 8 new compat mappings added (`humidity.fill`, `circle.inset.filled`, `diamond.fill`, `rainbow`, `powerplug`, `chart.line.uptrend.xyaxis`, `sun.and.horizon.fill`, `arrowshape.down.fill`); routed 11 direct-bypass call-sites where scene data structs held a `symbol: String` that was passed straight into `Image(systemName:)` / `Label(systemImage:)` (Ch.7 Scene 2 weather station, Ch.7 Scene 4 polar bear, Ch.13 Scene 3 graph, Ch.14 Scene 6 electromagnet, Ch.15 Scene 4 prism, Ch.19 Scene 5 tides, Ch.19 Scene 7 constellations, Ch.19 Scene 8 ISRO missions) |
...
| H2 | `.accessibilityHint` where non-obvious | ✅ H2 — `.accessibilityHint` added to actionable controls across the app (the 2026-06-11/12 H2 sweep alone added ~169 NEW hints across ~89 files; total `.accessibilityHint(` sites in the codebase today: ~326). Universal labels (Cancel / Done / Close / Back) intentionally hint-free. Verified by `check_a11y_labels` (**706/706 Buttons labeled, exact 100%**; ratchet floor bumped 90 → 99 on 2026-06-13. The prior 705/706 holdout — Scene1_SourOrBitter's choiceButton whose `.accessibilityLabel` sat past the lint's 1500-char window — was fixed in `afa10ee` by hoisting the 37-line action closure to a helper method) |
...
| H7 | Keyboard-only navigation full coverage | 🟡 every action has a menu Command (with shortcut) or a focused Button; full coverage relies on SwiftUI's default focus traversal. **v8 audit (2026-06-11):** confirmed broad coverage — 21 `keyboardShortcut`s in `desktopAhaanApp.swift`, a dedicated `KeyboardShortcutsSheet`, menu `Command`s; design intentionally uses default focus traversal (no `.focusable()`). Closing this fully needs an XCUITest driving the home flow with an **AX grant on the runner** (default CI is unit-only; UI tests are `--ui` opt-in) — verifiable on the iMac / an AX-granted runner, not headlessly on the dev Mac |
...
| J8 | Padding / spacing consistency via DesignTokens | ✅ J8 — padding / spacing / radius migrated to DesignTokens across the app (Waves 1–6; ~3,150 spacing + ~340 radius sites). Two new ratchets (`check_designtokens_spacing` / `check_designtokens_radius`) prevent regression. See `J8_DESIGN_TOKENS_LEDGER.md` |
...
| ID | Category | Status |
|----|----------|--------|
| T1 | Unit tests (`Testing` framework) | ✅ **~835 XCTest + 66 swift-testing methods across 121+ files** (last sweeping count 2026-06-13). 2026-05-24 additions: `RelatedChaptersStripTests` (7 tests covering Surface 4's static derivation — real-pack pin for Ch.19 cross-chapter targets, self-ref/unresolved filtering, count rollup, sort order), `PilotInteractiveSheetCoordinatorTests` (5 tests pinning the coordinator's defer behaviour and SheetKind id stability). Also: the Date-sensitive `testStreak_*` flake (the "retry the push once" rule that lived in CLAUDE.md / REMEDIATION_LOG) is now permanently fixed via `DataStore(streakCalendar:)` injection — commit `cb615c2` |
| T2 | UI tests (XCUIAutomation) | 🟡 42 XCUITest methods across 10 files in the target (+ `NavigationSmokeUITests.swift` on disk pending Xcode "Add Files…" → 11 files / 43 methods once wired). Crash-regression locks: `Crash1_TryDiscoverMode_Ch1` (C1) + `Crash_BeyondThenDiscover` (C2). Audit walker: `Surface_AuditWalker`. `GoldenPathUITests` (8) covers chapter-detail surface mount, ConceptMapView round-trip, Ch.1 leaf-tour, RelatedChaptersStrip visible/nav on Ch.7, **notebook write/persist/reopen**, **glossary open/dismiss**, **Ch.14 wire-tour**. **2026-06-02 — every propagated Science sandbox/tour + the HomeExperiments sheet now has a walk:** `PropagatedToursUITests` (Ch.2 digestive, Ch.10 alveolus, Ch.11 xylem, Ch.15 lens — open→first-stop→close→re-mount); `PropagatedSandboxesUITests` (Ch.1/4/5/6/7/8/9/13/16 Build-a-* — mount→explainer-toggle→state-flip); `HomeExperimentsUITests` (Try-at-Home open→first-experiment→dismiss→re-mount). Plus 15 maths-Discover walks. Every queried accessibility string is copied verbatim from source. Two **AX-free, no-build** ratchets guard these on dev Macs / CI: `check_critical_uitest_presence.py` (22-method REQUIRED manifest — rename/delete fails at commit) + `check_uitest_label_coverage.py` (each propagated-CTA / sandbox contract label must be referenced verbatim by a walk — catches label drift AND missing walks). UI tests stay `--ui` opt-in: `bash scripts/ci-build-test.sh --ui` adds the `-only-testing:desktopAhaanUITests/GoldenPathUITests` stage; the iMac (AX granted to `desktopAhaanUITests-Runner.app`) sets `export CI_BUILD_TEST_FLAGS=--ui`. Dev Macs / CI leave it unset and skip (clicks no-op without an AX grant → assertions fail by timeout, no false-confidence). **Authoritative green for the new walks is the iMac `--ui` run.** **Still 🟡 (not ✅): the Social Science bespoke interactives (`socialScienceInteractives`) have no smoke walk** — they use dynamic interpolated accessibility labels and lack a stable container id, so a grounded walk is fragile; deferred as gravy. That is the only remaining gap. See `UITEST_COVERAGE_CHECKPOINT.md` |
| T3 | Smoke test for navigation | 🟡 T3 — navigation smoke test source at `desktopAhaanUITests/NavigationSmokeUITests.swift` (single end-to-end walk: home → chapter → topic → concept → question). **pbxproj wiring NOW CONFIRMED** as of 2026-06-23 — `NavigationSmokeUITests.swift in Sources` is listed in the UITests target's Sources build phase (verified via `grep NavigationSmokeUITests desktopAhaan.xcodeproj/project.pbxproj`), so the file IS compiled and discoverable to the test runner. The standing T2 ratchets (`check_critical_uitest_presence` / `check_uitest_label_coverage`) cover the older smoke walks; once the iMac `--ui` run with an AX grant flips this surface green, this row goes ✅. Until then: 🟡 pending an AX-granted iMac run, NOT pending pbxproj work. See `IMAC_VERIFY_CHECKLIST.md` row T3. |
| T4 | Snapshot tests | 🟡 `Ch2_19_StructuralRatchetTests` ships a code-fingerprint snapshot equivalent — locks per-chapter topic count, concept count, question count, and 13 content-expansion item-counts. 9 routing-ratchet test classes (Beyond / CommonMistakes / Glossary / NcertQa / Scientists / WhatIf / MiniProject / SelfCheck / StoryMode added 2026-05-25/26) pin per-chapter article invariants. Image-based pixel snapshots remain unshipped (no 3rd-party dep allowed; XCUITest path needs AX grant on dev Mac). The fingerprint pattern catches the same regressions a pixel snapshot would for content drift |
...
| T6 | Pre-commit hooks | ✅ scripts/hooks/{pre-commit,pre-push} — try!/as! gate + ViewBuilder warn on commit; full ci-build-test.sh on push; install via scripts/install-git-hooks.sh. 2026-05-24: pre-push reads `CI_BUILD_TEST_FLAGS` env var and forwards to ci-build-test.sh, letting the iMac opt in to UI tests on every push without forking the hook. Install script prints the recipe (`export CI_BUILD_TEST_FLAGS=--ui`) on stdout so a fresh iMac clone surfaces the option immediately. 2026-05-31: the ViewBuilder check is now a real HARD gate (was inert — invoked with a `--quiet` flag argparse rejected, plus a commented-out `exit 1`) and a new `check_mainactor_closure_refs.py` hard gate (with `// mainactor-ok` escape hatch) blocks bare `@MainActor` method refs in `perform:`/`action:`/`on…:` args; both are driven by `test_lints.py --selftest` |
...
| Y3 | Page-ref backfill from textbook | ✅ closed 2026-06-23 (commit landing this row flip). Textbook Qs ship 100% `pageRefs`. Boss-quiz `pageRefs` backfilled 2026-05-25 (`179b28e`) — 200/200 Qs carry chapter-topic pages, locked by `BossQuizMigrationRatchetTests.testEveryBossQuizHasPageRefs`. Article-side cross-references (`relatedConceptIds`) wired throughout the 2026-05-26 enrichment templates. **Per-concept pageRefs audit completed 2026-06-23** (`CONTENT_SWEEP_REPORT.md`): every NEP textbook concept across maths (90/90), science (207/207), socialscience (293/293), and Sanskrit `sch01`–`sch15` carries precise `pageRefs` — 0 chapter-range-only entries remain. The 246 Sanskrit `ch01` concepts without `pageRefs` are the documented legacy-vocabulary-deck carve-out (per `CLAUDE.md`: "Sanskrit `ch01` carve-out: the legacy vocabulary deck stays at `ch01` and is intentionally exempt from the NEP cross-subject parity ratchets"); curated word lists, no textbook excerpt to reference. |
...
| Y5 | Advanced-tier test-paper coverage | ✅ (next free id after Y4) The `_Advanced_` tier was a 2-chapter prototype (Maths Ch15, Science Ch13) as of v8 start; **now complete for all 69 base chapters** (Maths 15/15, Science 19/19, Social Science 20/20, Sanskrit 15/15 — 69 foundation + 69 advanced = 138 registry papers, pinned by `OlympiadExamHallTests`). Triplet completeness hard-gated by `scripts/check_testpaper_triplet.py` (every `_QuestionPaper.md` ⇒ non-empty `_Solutions.md`; Resources stream also ⇒ `_SolvedGuide.html`). Per-chapter grid in `ADVANCED_TIER_LEDGER.md`; SolvedGuide.html auto-generated by `scripts/make_solved_guide.py --bulk`. Every Advanced paper: 60 single-correct MCQs, balanced 15/15/15/15 key, beyond-grade reach grounded in the chapter pack JSON + source PDFs |
...
| CL5 | `Color.compat*` palette not cataloged with WCAG contrast pairs | ✅ Option C (2026-05-20) — every BrandColor accent now has a deepened-hue RGB that hits WCAG AA against the Discover canvas; `scripts/check_wcag_contrast.py` is the audit harness (auto-run in pre-commit when BrandColor changes); 7 ChapterContentTests pin the contrast ratios as ratchets so a future tint can't drop below 4.5:1 silently |
...
| ID | Category | Status |
|----|----------|--------|
| LY1 | Discover scenes top-anchored with ~50% empty canvas below → wasted vertical real estate | 🟡 per-scene `Spacer()` placement decision — each scene's body chooses whether to expand interactive widgets vertically or anchor at top. Per-scene content judgment, not a chrome bug. Could be revisited per-scene as content refines, but isn't a systemic issue. 2026-05-19 sweep landed the canonical scene template (`ScrollView { LazyVStack(spacing: 14) { ... }.frame(maxWidth: .infinity).padding(.bottom, 12) }`) across all 19 chapters — dropped greedy `Spacer(minLength: 0)`s and outer `.frame(maxHeight: .infinity)` modifiers that were the converse bug (content pushed BELOW viewport, footer Prev/Next unreachable on shorter windows). Also closed the GeometryReader-collapse pattern that shrank diagrams to invisible dots (Ch.1 Scene 8 Nitrogen Cycle, Ch.7 Scene 4 Polar Bear, plus 6 others where the inner geo's `.frame(maxHeight: X)` cap let the GR collapse to its zero intrinsic) |
...
| ID | Category | Status |
|----|----------|--------|
| MO1 | Counter / score changes have no animation / scale-pop | ✅ DiscoverShell header "X done" counter (2026-05-19) now scale-pops to 1.18× via spring when `completedSceneIds.count` increases — the highest-visibility counter in the app (visible on every scene). Honours `accessibilityReduceMotion`. Pattern: `.scaleEffect` driven by a `@State counterPopActive: Bool` toggled in `.onChange(of: completedIds.count)`. Per-scene mini-counters (e.g. "Nitrogen absorbed: +1") remain per-scene content decisions; chrome counter now ships the effect for every chapter |
...
| MO4 | Reduce Motion respected by TimedScene + ParticleEmitter; spot animations still bypass (dup of H5/O4) | ✅ closed 2026-06-23 (commit `fbe0afc`). The dev-Mac audit pass swept every per-scene `withAnimation` call and either wrapped each in `withAnimationRespectingReduceMotion(...)` or applied a `reduceMotion ? .none : ...` ternary. Locked by the new `scripts/check_withanimation_motion.py` lint (LH005b) — pre-commit + ci-build-test enforced — so any future scene adding an unguarded `withAnimation { }` fails the gate. 5 sites fixed in that pass (Scene1_PlantKitchen:154, Scene4_HotSoupColdSpoon:172, Scene6_FourStomachsOfACow:158, AllChaptersCompleteOverlay:62/124). Repo-wide lint returns clean across 531 Swift files. |
...
| DM7 | Completion celebration absent (no confetti / "you finished Discover Mode") | ✅ AllChaptersCompleteOverlay (2026-05-19) — shipped as a private struct inside ContentView.swift (matches WelcomeSheet/KeyboardShortcutsSheet pattern; avoids pbxproj edit). Fires once when `DataStore.allDiscoverChaptersComplete` flips true (380th scene completed — was 171 before the 2026-05-21 expansion of every chapter from 9 → 20 scenes). Shows scaled-emoji + "You finished Discover Mode!" + total Boss Quiz score + confetti emitter. Dismissed via tap-outside or Continue button; suppressed thereafter via `AppStorageKeys.hasSeenAllChaptersCelebration`. Threshold + flag pinned in `ChapterContentTests.testTotalDiscoverScenesPinnedAt380` and `testAllDiscoverChaptersCompleteFlagFiresAtThreshold` |
...
| EM4 | No "all chapters complete" / "you've explored everything" celebration screen | ✅ shipped 2026-05-19 — see DM7 above. ContentView.swift hosts AllChaptersCompleteOverlay; fires when DataStore.discoverProgress.count ≥ 380 (was 171; bumped 2026-05-21 after every chapter expanded from 9 → 20 scenes). Includes Boss Quiz total score across all 19 chapters, confetti via ParticleEmitter, and one-tap dismissal that persists hasSeenAllChaptersCelebration so the overlay shows once per student |
...
| CT6 | Class-7-appropriate reading level not validated across all callouts | ✅ `scripts/check_callout_reading_level.py` (2026-05-19) — Flesch-Kincaid grade-level lint on every `LookingAheadCallout(...)` and `TryAtHomeCallout(...)` invocation across every `Scene*.swift`. Self-contained (no external dependency). Default threshold is grade 13 — the callouts are exam-prep with mature vocabulary by design (Class 11/12/NEET/JEE preview content), so 13 is the practical warning line; pure Class-7 prose would aim for ≤9. Baseline scan: 295 callouts found, 56 above 13 — all "Class 11/12/NEET preview" content as expected. The tool surfaces the distribution; doesn't hard-gate. Use it during future content edits to spot accidental complexity in casual-encouragement callouts |
