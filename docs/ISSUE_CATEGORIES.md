...
| H2 | `.accessibilityHint` where non-obvious | ✅ H2 — `.accessibilityHint` added to actionable controls across the app (~169 hints across ~89 files). Universal labels (Cancel / Done / Close / Back) intentionally hint-free. Verified by `check_a11y_labels` (100% Button label coverage, 706/706) |
...
| J8 | Padding / spacing consistency via DesignTokens | ✅ J8 — padding / spacing / radius migrated to DesignTokens across the app (Waves 1–5; ~3,150 spacing + ~340 radius sites). Two new ratchets (`check_designtokens_spacing` / `check_designtokens_radius`) prevent regression. See `J8_DESIGN_TOKENS_LEDGER.md` |
...
| T3 | Smoke test for navigation | ✅ T3 — navigation smoke test (`NavigationSmokeUITests.swift`) walks home → chapter → topic → concept → question end-to-end. AX-grant required to run; commit-time T2 ratchets (`check_critical_uitest_presence` / `check_uitest_label_coverage`) cover ratchet presence |
