...
| Y3 | Page-ref backfill from textbook | 🟡 textbook Qs ship 100% `pageRefs`. Boss-quiz `pageRefs` backfilled 2026-05-25 (`179b28e`) — 200/200 Qs carry chapter-topic pages, locked by `BossQuizMigrationRatchetTests.testEveryBossQuizHasPageRefs`. Article-side cross-references (`relatedConceptIds`) wired throughout the 2026-05-26 enrichment templates. Per-concept `pageRefs` audit remains for a future pass — some concepts cite chapter-level ranges rather than precise page numbers |
