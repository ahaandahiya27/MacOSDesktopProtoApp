...
| Y5 | Advanced-tier test-paper coverage | 🟡 (next free id after Y4) Olympiad/SolvedGuide base stream is broad (71 papers, all four subjects) but the `_Advanced_` tier was a 2-chapter prototype (Maths Ch15, Science Ch13) as of v8 start. Triplet completeness now hard-gated by `scripts/check_testpaper_triplet.py` (every `_QuestionPaper.md` ⇒ non-empty `_Solutions.md`; Resources stream also ⇒ `_SolvedGuide.html`). Per-chapter Advanced rollout tracked in `ADVANCED_TIER_LEDGER.md`; SolvedGuide.html auto-generated by `scripts/make_solved_guide.py --bulk`. Flips ✅ when all 69 base chapters carry an Advanced triplet |
