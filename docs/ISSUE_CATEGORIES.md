...
| T2 | UI tests (XCUIAutomation) | 🟡 11 tests across 4 files. Crash-regression locks: `Crash1_TryDiscoverMode_Ch1` (C1) + `Crash_BeyondThenDiscover` (C2). Audit walker: `Surface_AuditWalker`. 2026-05-24: `GoldenPathUITests` now ships 8 tests — chapter-detail surface mount, ConceptMapView round-trip, InsideTheLeafTour round-trip, RelatedChaptersStrip visible on Ch.7, Ch.7 → Ch.4 chip nav, **notebook open + write + persist + reopen (only test that asserts on user-data persistence)**, **glossary sheet open + dismiss**, **InsideTheWireTour open + dismiss (propagated tour pinned, complements the Ch.1 pilot leaf-tour test)**. UI tests are opt-in via `bash scripts/ci-build-test.sh --ui` — the flag adds an xcodebuild test stage running `-only-testing:desktopAhaanUITests/GoldenPathUITests`. The pre-push hook honours `CI_BUILD_TEST_FLAGS` env var so the iMac (where AX is granted to `desktopAhaanUITests-Runner.app`) gets `export CI_BUILD_TEST_FLAGS=--ui` in its shell profile and runs UI tests routinely. Dev Macs and CI runners without an AX grant leave the var unset and continue to skip. Crash-regression locks intentionally remain skipped even under `--ui` — they reproduce only on AMD R9 M290X + Big Sur. Still 🟡 (not ✅) because per-chapter sandbox/tour coverage is still partial — Ch.1 leaf tour + Ch.14 wire tour are sampled; the other 9 propagated sandboxes/tours (Ch.2 digestive, Ch.4 heat-flow, Ch.5 pH, Ch.6 reaction, Ch.7 climate, Ch.8 wind, Ch.9 soil, Ch.10 alveolus, Ch.11 xylem, Ch.13 motion, Ch.15 lens, Ch.16 water-cycle) lack walks. HomeExperiments sheet also untested. Filed as a thinning queue |
