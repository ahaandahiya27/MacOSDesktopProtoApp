...
| T2 | UI tests (XCUIAutomation) | 🟡 8 tests across 4 files. Crash-regression locks: `Crash1_TryDiscoverMode_Ch1` (C1) + `Crash_BeyondThenDiscover` (C2). Audit walker: `Surface_AuditWalker`. 2026-05-24: `GoldenPathUITests` — 5 tests covering the recently-shipped surfaces (Ch.1 chapter detail mounts every expected surface; ConceptMapView sheet open + dismiss; InsideTheLeafTour open + dismiss; RelatedChaptersStrip visible on Ch.7; Ch.7 → Ch.4 chip navigation). UI tests are opt-in via `bash scripts/ci-build-test.sh --ui` — the `--ui` flag adds a third xcodebuild test invocation that runs `-only-testing:desktopAhaanUITests/GoldenPathUITests`. The pre-push hook honours `CI_BUILD_TEST_FLAGS` env var so the iMac (where AX is granted to `desktopAhaanUITests-Runner.app`) gets `export CI_BUILD_TEST_FLAGS=--ui` in its shell profile and runs UI tests routinely. Dev Macs and CI runners without an AX grant leave the var unset and continue to skip. Crash-regression locks intentionally remain skipped even under `--ui` — they reproduce only on AMD R9 M290X + Big Sur. Still 🟡 (not ✅) because the broader coverage gap remains: notebook + home-experiments + glossary sheets aren't UI-tested yet, and per-chapter sandbox / tour CTAs are only sampled (Ch.1 leaf tour); the other 10 propagated tours / sandboxes lack walks. Filed as a queue, not a closed category |
