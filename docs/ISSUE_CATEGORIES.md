...
| T2 | UI tests (XCUIAutomation) | 🟡 8 tests across 4 files. Crash-regression locks: `Crash1_TryDiscoverMode_Ch1` (C1) + `Crash_BeyondThenDiscover` (C2). Audit walker: `Surface_AuditWalker`. New 2026-05-24: `GoldenPathUITests` — 5 tests covering the recently-shipped surfaces (Ch.1 chapter detail mounts every expected surface; ConceptMapView sheet open + dismiss; InsideTheLeafTour open + dismiss; RelatedChaptersStrip visible on Ch.7; Ch.7 → Ch.4 chip navigation). UI tests deliberately skipped from `scripts/ci-build-test.sh` via `-skip-testing:desktopAhaanUITests` because they need an Accessibility grant on the runner; explicit run on the iMac via `xcodebuild test -only-testing:desktopAhaanUITests`. Still 🟡 (not ✅) because the broader coverage gap remains: notebook + home-experiments + glossary sheets aren't UI-tested yet, and per-chapter sandbox / tour CTAs are only sampled (Ch.1 leaf tour); the other 10 propagated tours / sandboxes lack walks. Filed as a queue, not a closed category |
