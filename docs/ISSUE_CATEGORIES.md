...
| T3 | Smoke test for navigation | 🟡 T3 — navigation smoke test source written at `desktopAhaanUITests/NavigationSmokeUITests.swift` (single end-to-end walk: home → chapter → topic → concept → question). **Not yet wired into the UITests target's `project.pbxproj`** — pbxproj edits must be done via Xcode's "Add Files…" dialog per the never-edit-pbxproj invariant; flips to ✅ once the file is in the target and the iMac `--ui` run is green. AX-grant required to execute on the runner. Existing T2 ratchets (`check_critical_uitest_presence` / `check_uitest_label_coverage`) gate presence of the older smoke walks; a follow-up commit will add `NavigationSmokeUITests/test_homeToQuestionDetail_endToEnd` to the critical-flow manifest once the file is in the target |
